...
import uno.meng.ner.dictionary.CoreDictionary;
...
public class IOUtil {
  ...
  public static boolean saveObjectTo(Object o, String path) {
    try {
      ...
      System.out.println("在保存对象" + o + "到" + path + "时发生异常" + e);
      return false;
    }
  ...
  public static Object readObjectFrom(String path) {
    ...
    try {
      ...
      System.out.println("在从" + path + "读取对象时发生异常" + e);
    }
  ...
  public static String readTxt(String path) {
    ...
    try {
      ...
      System.out.println("找不到" + path + e);
      return null;
    } catch (IOException e) {
      System.out.println("读取" + path + "发生IO异常" + e);
      return null;
    }
  }
  ...
  public static boolean saveTxt(String path, String content) {
    try {
      ...
      System.out.println("IOUtil saveTxt 到" + path + "失败" + e.toString());
      return false;
    }
    return true;
  }
  ...
  public static byte[] readBytes(String path) {
    try {
      ...
      System.out.println("读取" + path + "时发生异常" + e);
    }
  ...
  public static String readTxt(String file, String charsetName) throws IOException {
    ...
    int len;
    int off = 0;
    while ((len = is.read(targetArray, off, targetArray.length - off)) != -1
        && off < targetArray.length) {
      off += len;
    }
    is.close();
  ...
  public static int readBytesFromOtherInputStream(InputStream is, byte[] targetArray)
    ...
    int len;
    int off = 0;
    while (off < targetArray.length
        && (len = is.read(targetArray, off, targetArray.length - off)) != -1) {
      off += len;
    }
    return off;
  }
  ...
  public static LinkedList<String> readLineListWithLessMemory(String path) {
    ...
    try {
      ...
      System.out.println("加载" + path + "失败，" + e);
    }
  ...
  public static boolean saveEntrySetToTxt(Set<Map.Entry<Object, Object>> entrySet, String path,
      String separator) {
  ...
  public static class LineIterator implements Iterator<String>, Iterable<String> {
    ...
    public LineIterator(BufferedReader bw) {
      ...
      try {
        ...
        System.out.println("在读取过程中发生错误" + TextUtility.exceptionToString(e));
        bw = null;
      }
    }
    ...
    public LineIterator(String path) {
      try {
        ...
        System.out.println("文件" + path + "不存在，接下来的调用会返回null\n" + TextUtility.exceptionToString(e));
        bw = null;
      } catch (IOException e) {
        System.out.println("在读取过程中发生错误" + TextUtility.exceptionToString(e));
        bw = null;
    ...
    public void close() {
      ...
      try {
        ...
        System.out.println("关闭文件失败" + TextUtility.exceptionToString(e));
      }
      return;
    }
  ...
  public static void loadDictionary(BufferedReader br,
      TreeMap<String, CoreDictionary.Attribute> storage) throws IOException {
    String line;
  ...
  public static void writeCustomNature(DataOutputStream out,
      LinkedHashSet<Nature> customNatureCollector) throws IOException {
