package uno.meng.ner.trie;

import uno.meng.ner.AhoCorasick.AhoCorasickDoubleArrayTrie;
import uno.meng.ner.corpus.io.ByteArray;
import uno.meng.ner.corpus.io.ByteArrayStream;
import uno.meng.ner.corpus.io.IIOAdapter;
import uno.meng.ner.corpus.io.IOUtil;
import uno.meng.ner.utility.ByteUtil;
import java.io.*;
...
public class DoubleArrayTrie<V> implements Serializable, ITrie<V> {
  ...
  public void open(String fileName) throws IOException {
    ...
    try {
      is = new DataInputStream(new BufferedInputStream(IOUtil.newInputStream(fileName), BUF_SIZE));
  ...
  private boolean loadBaseAndCheck(String path) {
    try {
      DataInputStream in = new DataInputStream(new BufferedInputStream(
          IOAdapter == null ? new FileInputStream(path) : IOAdapter.open(path)));
      size = in.readInt();
      base = new int[size + 65535]; // 多留一些，防止越界
      check = new int[size + 65535];
      for (int i = 0; i < size; i++) {
  ...
  public LinkedList<Map.Entry<String, V>> commonPrefixSearchWithValue(char[] keyChars, int begin) {
    ...
    for (int i = begin; i < len; ++i) {
      ...
      if (b == check[p] && n < 0) // base[p] == check[p] && base[p] < 0 查到一个词
      {
        result.add(new AbstractMap.SimpleEntry<String, V>(new String(keyChars, begin, i - begin),
            v[-n - 1]));
      }
    ...
    if (b == check[p] && n < 0) {
      result.add(new AbstractMap.SimpleEntry<String, V>(new String(keyChars, begin, len - begin),
          v[-n - 1]));
    }
