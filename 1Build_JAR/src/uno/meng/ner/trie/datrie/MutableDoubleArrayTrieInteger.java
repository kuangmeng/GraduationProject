package uno.meng.ner.trie.datrie;
...
public class MutableDoubleArrayTrieInteger
    implements Serializable, Iterable<MutableDoubleArrayTrieInteger.KeyValuePair>, ICacheAble {
  ...
  public int[] transferValues(int state, int codePoint) {
    ...
    for (int i = 0; i < ids.length; i++) {
      int c = ids[i];
      if ((getBase(state) + c < getBaseArraySize()) && (getCheck(getBase(state) + c) == state)) {
    ...
    if (getCheck(getBase(state) + UNUSED_CHAR_VALUE) == state) {
      int value = getLeafValue(getBase(getBase(state) + UNUSED_CHAR_VALUE));
  ...
  public void save(DataOutputStream out) throws IOException {
    if (!(charMap instanceof Utf8CharacterMapping)) {
      System.out.println("将来需要在构造的时候传入 " + charMap.getClass());
    }
    out.writeInt(size);
    base.save(out);
    check.save(out);
  ...
  public boolean load(ByteArray byteArray) {
    ...
    if (!base.load(byteArray))
      return false;
    if (!check.load(byteArray))
      return false;
    return true;
  }
