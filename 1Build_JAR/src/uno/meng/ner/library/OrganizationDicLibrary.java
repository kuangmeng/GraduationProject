...
public class OrganizationDicLibrary {

  public static final String DEFAULT = "organition_dic";
  ...
  // 用户自定义词典
  private static final Map<String, MengSegMap<String, Forest>> DIC = new HashMap<>();
  ...
  static {
    ...
    putIfAbsent(DEFAULT, "data/dictionary/organization/organization.dic");
  ...
  public static Forest get(String key) {
    ...
    MengSegMap<String, Forest> kv = DIC.get(key);
    ...
    if (kv == null) {
      ...
      System.out.println("dic " + key + " not found in config ");
      return null;
    }
  ...
  private synchronized static Forest init(String key, MengSegMap<String, Forest> kv,
      boolean reload) {
    Forest forest = kv.getV();
    if (forest != null) {
      if (reload) {
    ...
    try {
      // System.out.println("begin init dic !");
      ...
      try (BufferedReader br = IOUtil.getReader(PathToStream.stream(kv.getK()), "UTF-8")) {
        while ((temp = br.readLine()) != null) {
          if (StringUtil.isNotBlank(temp)) {
            ...
            // 如何核心辞典存在那么就放弃
            if (DATDictionary.getId(strs[0]) > 0) {
              continue;
            }
            if (strs.length != 3) {
      ...
      // System.out.println("load dic use time:" + (System.currentTimeMillis() - start) + " path is
      // : " + kv.getK());
      kv.setV(forest);
      return forest;
    } catch (Exception e) {
      System.out.println("Init dic library error :" + e.getMessage() + ", path: " + kv.getK());
      DIC.remove(key);
      return null;
    }
  }
  ...
  public static void put(String key, String path, Forest forest) {
    DIC.put(key, MengSegMap.with(path, forest));
  ...
  public static void putIfAbsent(String key, String path) {
    ...
    if (!DIC.containsKey(key)) {
      DIC.put(key, MengSegMap.with(path, (Forest) null));
    }
  }
  ...
  public static MengSegMap<String, Forest> remove(String key) {
    MengSegMap<String, Forest> kv = DIC.get(key);
    if (kv != null && kv.getV() != null) {
      kv.getV().clear();
    }
