...
public class ExtractingTask implements Runnable {
  ...
  private boolean _validate(Token token, Term term) {
    ...
    if ((!terms.contains(Token.ALL)) && !terms.contains(term.getName())
        && !(terms.contains(":" + term.getNatureStr()))) {
      return true;
    }
