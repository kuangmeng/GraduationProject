...
import uno.meng.ner.FinanceDicExtractNER;
import uno.meng.ner.LawDicExtractNER;
...
public class LawRecognition {

  public static Result segment(String str) {
    return LawDicExtractNER.parse(str);
  }
  ...
  public static List<Term> extractLaw(String str) {
    List<Term> result = new ArrayList<>();
    List<Term> list = segment(str).getTerms();
    if (list.size() > 0)
      result.addAll(list.stream().filter(term -> (term.getNatureStr().equals("law")))
