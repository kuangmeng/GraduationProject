...
import uno.meng.ner.utility.Predefine;
import uno.meng.ner.utility.TextUtility;
import java.io.*;
import java.util.*;
...
public class CoreDictionary {
  ...
  public final static String path = uno.meng.Constants.CoreDictionaryPath;
  ...
  private static boolean load(String path) {
    System.out.println("核心词典开始加载:" + path);
    if (loadDat(path))
      return true;
    ...
    try {
      ...
      System.out.println("核心词典读入词条" + map.size() + " 全部频次" + MAX_FREQUENCY + "，耗时"
      ...
      System.out.println("核心词典" + path + "不存在！" + e);
      return false;
    } catch (IOException e) {
      System.out.println("核心词典" + path + "读取错误！" + e);
      return false;
    }
  ...
  static boolean loadDat(String path) {
    try {
      ...
      System.out.println("读取失败，问题发生在" + e);
      return false;
    }
    return true;
  }
