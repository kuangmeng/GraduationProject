...
public class CrfLearn {
  public static boolean run(String[] args) {
    ...
    options.addOption("m", "maxiter", true,
    ...
    options.addOption("T", "convert_to_text", true, "convert binary model to text model");
    options.addOption("t", "textmodel", false, "build also text model file for debugging");
    options.addOption("a", "algorithm", true,
        "(CRF|CRF-L1|CRF-L2|MIRA)\", \"select training algorithm");
    options.addOption("p", "thread", true, "number of threads(default auto detect)");
    options.addOption("H", "shrinking-size", true,
        "set INT for number of iterations variable needs to be optimal before considered for shrinking. (default 20)");
    ...
    if (convert) {
      ...
      if (!encoder.learn(restArgs[0], restArgs[1], restArgs[2], textmodel, maxiter, freq, eta, C,
          threadNum, shrinkingSize, algo)) {
