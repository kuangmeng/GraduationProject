...
public class LbfgsOptimizer {
  ...
  int lbfgs_optimize(int size, int msize, double[] x, double f, double[] g, double[] diag,
      double[] w, boolean orthant, double C, double[] v, double[] xi, int iflag) {
    double yy = 0.0;
    double ys = 0.0;
    int bound = 0;
    int cp = 0;
  ...
  public int optimize(int size, double[] x, double f, double[] g, boolean orthant, double C) {
    ...
    if (orthant) {
      ...
      iflag = lbfgs_optimize(size, msize, x, f, g, diag_, w_, orthant, C, v_, xi_, iflag_);
      iflag_ = iflag;
    } else {
      iflag = lbfgs_optimize(size, msize, x, f, g, diag_, w_, orthant, C, g, xi_, iflag_);
      iflag_ = iflag;
    }
