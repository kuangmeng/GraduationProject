...
public class Mcsrch {
  ...
  void mcsrch(int size, double[] x, double f, double[] g, double[] s, int startOffset, double[] stp,
    ...
    while (true) {
      if (!firstLoop || (firstLoop && info[0] != -1)) {
        ...
        if ((brackt
            && ((stp[0] <= stmin || stp[0] >= stmax) || nfev[0] >= maxfev - 1 || infoc == 0))
      ...
      if (stage1 && f <= fx && f > ftest1) {
        ...
        int[] infocArr = {infoc};
        mcstep(stxArr, fxmArr, dgxmArr, styArr, fymArr, dgymArr, stp, fm, dgm, bracktArr, stmin,
            stmax, infocArr);
        stx = stxArr[0];
        fxm = fxmArr[0];
        dgxm = dgxmArr[0];
        ...
        boolean[] bracktArr = {brackt};
        int[] infocArr = {infoc};
        mcstep(stxArr, fxArr, dgxArr, styArr, fyArr, dgyArr, stp, f, dg, bracktArr, stmin, stmax,
            infocArr);
        stx = stxArr[0];
        fx = fxArr[0];
        dgx = dgxArr[0];
