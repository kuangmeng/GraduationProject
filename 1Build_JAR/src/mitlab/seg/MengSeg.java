...
import mitlab.seg.crf_seg.util.MatrixUtil;
...
import mitlab.seg.rule.Match;
import mitlab.seg.rule.Rule;
...
public class MengSeg {
  ...
  public static List<Rule> rule = new ArrayList<Rule>();
  public MengSeg(Model model) throws IOException {
    segmenter = new PerceptronSegmenter(mitlab.seg.Constants.CWSMODEL);
    this.model = model;
  };
  
public void addRule(String ru){
	  String[] ret = ru.split("\\s+");
	  Rule r = new Rule();
  ...
  public List<String> Seg(String line){
    ...
    for(Rule ru : rule){
    		List<Integer> loc = Match.getLocation(line, ru.toString());
    		if(loc.get(0) != -1){
    			for(int l : loc){
    		          result = Match.match(line,result,ru,l);
    		    	}
    		}
    }
...
//    ret = segTemp(ret, fs, len);
    switch (flag) {
      ...
      case 3:
        ...
        break;
      case 4:
        List<Term> finance1 = FinanceRecognition.extractFinance(line);
        ret = segTemp(tmp, finance1, len);
        List<Term> law1 = LawRecognition.extractLaw(line);
        ret = segTemp(ret, law1, len);
        List<Term> medicine1 = MedicineRecognition.extractSick(line);
        ret = segTemp(ret, medicine1, len);
        break;
      default:
        ret = tmp;
        break;
