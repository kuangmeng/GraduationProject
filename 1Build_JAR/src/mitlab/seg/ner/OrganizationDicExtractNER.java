...
import mitlab.seg.ner.library.OrganizationDicLibrary;
...
public class OrganizationDicExtractNER extends Extract_NER {
  ...
  protected List<Term> getResult(final Graph graph) {
    ...
    Merger merger = new Merger() {
      ...
      public List<Term> merger() {
        // 用户自定义词典的识别
        userDefineRecognition(graph, OrganizationDicLibrary.get());
        graph.walkPath();
        return getResult();
      }
  ...
  public OrganizationDicExtractNER() {
    super();
  }

  public OrganizationDicExtractNER(Reader reader) {
    super.resetContent(new IOReader(reader));
  }

  public static Result parse(String str) {
    return new OrganizationDicExtractNER().parseStr(str);
  }

  public static Result parse(String str, Forest... forests) {
    return new OrganizationDicExtractNER().setForests(forests).parseStr(str);
  }
}
