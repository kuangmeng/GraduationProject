...
public class FinanceDicExtractNER extends Extract_NER {
  ...
  protected List<Term> getResult(final Graph graph) {
    ...
    Merger merger = new Merger() {
      ...
      public List<Term> merger() {
        // 用户自定义词典的识别
        userDefineRecognition(graph, FinanceDicLibrary.get());
        graph.walkPath();
        return getResult();
      }
  ...
  public FinanceDicExtractNER() {
    super();
  }

  public FinanceDicExtractNER(Reader reader) {
    super.resetContent(new IOReader(reader));
  }

  public static Result parse(String str) {
    return new FinanceDicExtractNER().parseStr(str);
  }

  public static Result parse(String str, Forest... forests) {
    return new FinanceDicExtractNER().setForests(forests).parseStr(str);
  }
}
