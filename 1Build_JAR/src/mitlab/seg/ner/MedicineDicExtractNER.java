...
import mitlab.seg.ner.library.MedicineDicLibrary;
...
public class MedicineDicExtractNER extends Extract_NER {
  ...
  protected List<Term> getResult(final Graph graph) {
    ...
    Merger merger = new Merger() {
      ...
      public List<Term> merger() {
        // 用户自定义词典的识别
        userDefineRecognition(graph, MedicineDicLibrary.get());
        graph.walkPath();
        return getResult();
      }
  ...
  public MedicineDicExtractNER() {
    super();
  }

  public MedicineDicExtractNER(Reader reader) {
    super.resetContent(new IOReader(reader));
  }

  public static Result parse(String str) {
    return new MedicineDicExtractNER().parseStr(str);
  }

  public static Result parse(String str, Forest... forests) {
    return new MedicineDicExtractNER().setForests(forests).parseStr(str);
  }
}
