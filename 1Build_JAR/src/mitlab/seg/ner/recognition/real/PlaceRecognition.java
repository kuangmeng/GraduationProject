...
import mitlab.seg.ner.PlaceDicExtractNER;
...
public class PlaceRecognition {
  ...
  public static Result segment(String str) {
    return PlaceDicExtractNER.parse(str);
  }
  ...
  public static List<Term> extractPlace(String str) {
    List<Term> result = new ArrayList<>();
    List<Term> list = segment(str).getTerms();
    if (list.size() > 0)
      result.addAll(list.stream().filter(term -> term.getNatureStr().equals("place"))
