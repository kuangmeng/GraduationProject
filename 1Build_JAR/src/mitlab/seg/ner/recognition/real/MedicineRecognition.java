...
import mitlab.seg.ner.MedicineDicExtractNER;
...
public class MedicineRecognition {

  public static Result segment(String str) {
    return MedicineDicExtractNER.parse(str);
  }
  ...
  public static List<Term> extractSick(String str) {
    List<Term> result = new ArrayList<>();
    List<Term> list = segment(str).getTerms();
    if (list.size() > 0)
      result.addAll(list.stream().filter(term -> term.getNatureStr().equals("sick"))
