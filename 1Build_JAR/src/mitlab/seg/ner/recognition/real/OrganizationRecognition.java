...
import mitlab.seg.ner.OrganizationDicExtractNER;
...
public class OrganizationRecognition {
  ...
  public static Result segment(String str) {
    return OrganizationDicExtractNER.parse(str);
  }
  ...
  public static List<Term> extractOrganization(String str) {
    List<Term> result = new ArrayList<>();
    List<Term> list = segment(str).getTerms();
    if (list.size() > 0)
      result.addAll(list.stream().filter(term -> term.getNatureStr().equals("organization"))
