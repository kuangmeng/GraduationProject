...
import mitlab.seg.ner.library.PlaceDicLibrary;
...
public class PlaceDicExtractNER extends Extract_NER {
  ...
  protected List<Term> getResult(final Graph graph) {
    ...
    Merger merger = new Merger() {
      ...
      public List<Term> merger() {
        // 用户自定义词典的识别
        userDefineRecognition(graph, PlaceDicLibrary.get());
        graph.walkPath();
        return getResult();
      }
  ...
  public PlaceDicExtractNER() {
    super();
  }

  public PlaceDicExtractNER(Reader reader) {
    super.resetContent(new IOReader(reader));
  }

  public static Result parse(String str) {
    return new PlaceDicExtractNER().parseStr(str);
  }

  public static Result parse(String str, Forest... forests) {
    return new PlaceDicExtractNER().setForests(forests).parseStr(str);
  }
}
